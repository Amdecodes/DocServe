...
async function main() {
  ...
  try {
    ...
    await prisma.$executeRawUnsafe(
      `GRANT ALL ON TABLE "print_products" TO postgres, anon, authenticated, service_role;`,
    );

    // Grant all on print_orders
    await prisma.$executeRawUnsafe(
      `GRANT ALL ON TABLE "print_orders" TO postgres, anon, authenticated, service_role;`,
    );
