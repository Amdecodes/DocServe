  ...
  return (
    <div className="flex flex-col h-[calc(100vh-64px)] overflow-hidden">
      ...
      <div className="lg:hidden bg-white border-b flex">
        <button
          ...
          className={`flex-1 py-3 text-sm font-medium border-b-2 ${
            activeTab === "edit" ? "border-teal-600 text-teal-600" : "border-transparent text-gray-500"
          }`}
        >
          Edit Form
        </button>
        <button
          ...
          className={`flex-1 py-3 text-sm font-medium border-b-2 ${
            activeTab === "preview" ? "border-teal-600 text-teal-600" : "border-transparent text-gray-500"
          }`}
        >
          Live Preview
        </button>
      ...
      <div className="flex flex-1 lg:flex-row overflow-hidden relative">
        {/* Left: Form Area */}
        <div className={`w-full lg:w-3/5 bg-gray-50 flex flex-col h-full ${
            activeTab === 'preview' ? 'hidden lg:flex' : 'flex'
        }`}>
            {/* Stepper Header (Fixed at top) */}
            <div className="bg-white border-b px-4 py-4 md:px-6 overflow-x-auto shrink-0 z-10">
              <Stepper steps={STEPS} currentStep={currentStep} className="min-w-max" />
            </div>
            ...
            {currentStep === STEPS.length - 1 ? (
                <Link href="/checkout" className="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-green-600 text-primary-foreground hover:bg-green-700 h-11 rounded-md px-8">
                    {t("proceedButton")}
                </Link>
            ) : (
        ...
        {/* Right: Preview Area */}
        <div className={`w-full lg:w-2/5 bg-gray-100 lg:bg-gray-200 p-4 lg:p-8 border-l border-gray-300 relative overflow-y-auto ${
