...
6. Frontend initiates payment with tx_ref
   ↓
7. Order status updates: DRAFT → PENDING → PAID/FAILED
```
...
- ✅ Always validate `service_type` on server
- ✅ Never trust frontend-provided order IDs
- ✅ Always fetch order from database before processing payment
- ✅ Log all order creation attempts
- ✅ Monitor for duplicate tx_ref attempts
